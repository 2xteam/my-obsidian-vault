...
FitLog 초안이 **3062자**였는데, 프로젝트 46건의 본문 중간값은 1277자이고
대표 8건은 **1349~2275자** 대역이다. 손으로 쓰면 아는 것을 다 적게 되어
혼자 두 배가 된다. 상세 화면은 45건과 같은 4단 구조를 전제로 조판하므로
**형식과 분량을 함께** 맞춘다 (2026-09-09, 1968자로 줄였다).

두 가지를 나눠 적는다 —

- `성과 및 결과` 는 **무엇을 만들었고 무엇이 남았는지**만 적는다. 개발 중에
  방향을 바꾼 이야기는 넣지 않는다
- `회고` 는 **어떤 문제를 어떻게 풀었는지**를 적는다. 방향을 바꾼 이야기는
  여기 있어야 읽힌다

### 소개와 연락처도 손등록으로 덮어쓴다

`content/profile-manual.json` 의 `body` · `socialLinks` 가 있으면 Notion 에서
만든 것을 대체한다. 파일이 없으면 예전대로 동작한다.

Notion 루트 문서는 **페이지 머리말**로 쓰인 글이다("반갑습니다! 웹 프로그래머,
장민 입니다." 와 `>` 인용 나열). 포트폴리오의 소개 자리에 그대로 놓으면
어색하다. 생년월일과 MBTI 도 뺐다 — 생년월일은 전체 공개 사이트에 둘 이유가
없는 개인정보고, MBTI 는 이력 정보가 아니다.

⚠️ **이메일은 손등록하지 않는다.** 이력서 첫 줄에서 계속 자동으로 뽑아
링크 맨 앞에 놓는다. 두 곳에 적으면 한쪽만 고치는 일이 생긴다.

⚠️ **소개나 링크를 고치면 답변 캐시가 낡는다.** 인스타그램을 세 계정으로
바꿨을 때 `answers` 의 `contact` 가 옛 주소를 그대로 돌려줬다. 한 건만
다시 만들 수 있다 — `pnpm warm -- --write --force --key=contact --lang=ko`.
...
### ⚠️ `ingest --write` 가 이미지 링크를 지웠다 (2026-09-11)

이력 화면에 그림이 하나도 없다는 것을 사용자가 발견했다. 102건 중 이미지가
있는 문서는 **FitLog 하나뿐**이었고, 나머지 45건은 전부 비어 있었다.
**R2 파일은 86개 그대로 있었다** — 문서와의 연결만 끊긴 것이다.

원인은 두 스크립트의 역할이 겹친 데 있다.

- `ingest` 는 Notion 마크다운에서 이미지를 떼어내고 문서를 `images: []` 로
  만든 뒤 **`$set` 으로 통째로 덮는다**
- 실제 그림은 `images:migrate` 가 Notion 에서 받아 R2 에 올린 뒤 써 넣는다

그래서 **`ingest --write` 를 돌릴 때마다 이관 결과가 지워진다.** 순서를
`ingest` → `images:migrate` 로 적어 둔 이유가 이것인데, 소개문과 갤러리를
고치느라 `ingest` 만 두 번 돌리고 뒤를 빠뜨렸다.

**고친 방식** — 빈 배열로 덮지 않는다. `images` 를 `$set` 에서 빼고,
값이 있을 때만 넣는다. 처음 만들 때만 `$setOnInsert` 로 빈 배열을 둔다.

> 여기서 빈 배열은 "이미지가 없다"가 아니라 **"아직 모른다"**는 뜻이다.
> 아는 쪽(손등록·이관)이 채우고, 모르는 쪽은 손대지 않는다.

적재가 끝나면 **비어 있는 것을 세어 알려준다.** 예전에는 조용히 비어서,
사람이 화면을 보고 나서야 알았다.

```
! 이미지가 비어 있는 문서 45건 (Notion 에 그림이 있는 문서)
  채우려면:  pnpm images:migrate -- --write
```

다시 채우는 것은 안전하고 싸다. 키에 **내용 해시**가 들어가서 같은 그림은
같은 키가 되고, `HeadObject` 로 건너뛴다 — 실제로 46건이 전부
`올림 0 · 건너뜀 N` 이었다. Notion 왕복만 다시 한다.
