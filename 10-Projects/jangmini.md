...
> **2026-09-08 현재 — Phase 0~4 완료, Phase 5(방어·로그인·admin) 남음.**
> 배포는 살아 있고 문서 페이지는 동작하지만 **AI 채팅은 의도적으로 닫혀 있다**
> (`CHAT_ENABLED` 킬스위치). 이어받는 방법은 [[D jangmini 구축]]에 있다.
...
| 세션 쿠키 | `snap_user` · `.myjane.co.kr` | `jangmini_reader` · **호스트 전용** (Phase 5) |
| 로그인의 뜻 | 서비스를 쓰려면 필요 | **제한 해제일 뿐** — 익명도 전부 볼 수 있다 |
| admin | 포털 통합 admin | **이 앱 자체 라우트** `/admin` (Phase 5) |
...
이미지 이관 · 임시 차단장치를 더했다. 남은 것은 reader 로그인 · admin ·
답변 캐시 · 방어 7층.
...
- `CHAT_ENABLED` 는 **아직 어디에도 넣지 않았다.** Phase 5 가 끝나면 넣는다
...
| `readers_history` | 0 | 질문 이력 **겸 카운터** (Phase 5) |
| `login_attempts` | 0 | 로그인 실패. TTL 1시간 (Phase 5) |
| `suggestions` | 0 | 추천 질문 (Phase 5 — 지금은 `src/lib/suggestions.ts` 가 원본) |
| `answers` | 0 | 사전 생성 답변 캐시 (Phase 5) |
| `usage` | 0 | 일별 집계 — 전역 예산 캡 판정 (Phase 5) |
...
/api/chat          POST. **지금은 CHAT_ENABLED 로 닫혀 503**
```
...
- [ ] **Phase 5 — 답변 캐시 · 방어 7층 · reader 로그인 · admin · 채팅 개방**
      → [[D jangmini 구축]]
