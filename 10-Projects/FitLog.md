...
- [ ] **동의를 한 번에 받기.** 사진을 처음 등록할 때 허용 화면이 **두 번**
      나온다. `health` 를 받고 돌아오면 `overseas` 게이트를 다시 만난다.
      여섯 앱 공통 화면을 고치는 일이라 계획은 → [[C 법적 페이지]] 에 적어 뒀다
- [ ] **`.env.example` 에 `SESSION_SECRET` 추가.** 아래 사고의 직접적인
      재발 방지책이다. 여섯 앱 공통 값이라는 설명도 함께 적는다
- [ ] **설정 누락이 빈 500 으로 나오지 않게.** 지금은 본문이 비어서 화면이
      이유를 못 보여주고 "실패" 만 뜬다. `isOpenAiKeyConfigured` → 503 JSON
      처럼 시크릿 누락도 이유가 담긴 응답으로 떨어지게 한다
- [ ] **SnapWord 의 `SESSION_SECRET` 확인.** 코드는 참조하는데 로컬에
      Vercel 링크가 없어 확인하지 못했다. 나머지 넷(myjane · 2hbk ·
      snapnote · typelog)은 등록돼 있다
