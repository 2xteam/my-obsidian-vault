...
## 루트는 소개 페이지 (2026-09-03)

`/`가 곧 로그인 화면이라 이 앱이 무엇을 하는 곳인지 알 방법이 없었다.
로그인은 `/login`으로 옮기고 루트를 **로그인 없이 볼 수 있는 소개 페이지**로 바꿨다.
`(app)` 화면은 `components/AuthGate.tsx`가 지킨다 → [[앱 공통 UI와 아이콘]]
