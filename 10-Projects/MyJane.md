...
**라이트 전용**이다. 2026-09-06부터 **살구 톤**을 쓴다 —
크림 `#fdf6f0` / 살구 `#ef8666` / 골드 `#c9a84c` → [[살구 톤 팔레트]]
그 전에는 결쩜사 팔레트(`#fdfbff` / `#7c3aed`)를 그대로 썼는데 색까지 같아서 너무 닮았다.
**구조는 결쩜사를 그대로 두고 색만 갈았다** — 시트 쌓기, 어두운 푸터로 닫기.
다크를 얹지 않는다. 크림 바탕의 따뜻함이 사라지면 흔한 다크 사이트가 된다.
서체는 본문 Pretendard + 헤드라인 Gowun Batang 700.

⚠️ 살구는 밝아서 **글자로 쓸 수 없다**(크림 위 2.37:1). 글자·테두리는 `--accent-ink`
`#a64b2f`, 살구 버튼 위 글자는 흰색이 아니라 먹 `#2c1f1a` 다. 자세한 이유와
대비 수치는 [[살구 톤 팔레트]]에 있다.

상단·푸터 워드마크는 `my`(본문색) + `jane`(살구 잉크). 푸터는 어두우니 `my`가 크림이다
→ [[앱 공통 UI와 아이콘]]
