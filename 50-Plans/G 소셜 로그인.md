...
- [ ] 네이버 검수 **재요청** — 첫 검수는 활용처 증빙 미제출로 반려. `naver-review-evidence.png` 첨부
...
### 네이버 검수 반려 — "제공 정보 활용처" 증빙 (2026-09-10)

첫 검수는 **"이메일·이름이 실제로 쓰이는 화면 캡처"** 를 요구하며 반려됐다. 관리자 회원 목록 캡처는 안 된다 —
이용자 화면이 아니고 다른 회원 정보가 노출된다. 동의창·개발자센터 캡처도 반려 사유라고 명시돼 있다.
`myjane/scripts/naver-review-evidence.mjs` 가 실제 화면 셋을 찍어 항목별 활용 목적을 붙인 이미지 한 장을 만든다
(`docs/marketing/naver-review-evidence.png` · `.gitignore` — 실제 회원 이름·이메일이 들어간다).

- ① `/signup/social` — 네이버 이름이 "이름(별칭)" 칸에, 네이버 이메일이 계정 이메일로 (서명 `oauth_signup` 쿠키를 스크립트가 만들어 넣는다)
- ② `/login` — 이메일이 로그인 아이디 (**세션 없는 컨텍스트**로 찍는다 — 세션이 있으면 홈으로 튄다)
- ③ FitLog `/my` — "OO님의 프로필" 이름 표시
- 아래 표: 이메일=계정 식별·비밀번호 찾기·인증 메일·계정 연결 / 이름=화면 표시·참가자 이름 / 저장하지 않는 것=토큰·사진

네이버 개발자센터 → 애플리케이션 → **검수 요청** 의 "제공 정보 활용처" 첨부에 이 PNG 하나를 올린다.
포털·fitlog verify 서버(3010·3013)가 떠 있어야 하고, 실제 네이버 회원이 없으면 예시 값으로 그린다.
