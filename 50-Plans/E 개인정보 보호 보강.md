...
updated: 2026-09-09
status: 2차 배포 (2026-09-09) — 이메일 전용 로그인 · 전환 · 이행기 종료. 운영 확인 남음
...
## 2차 — 2026-09-09 (사용자 요청 9건)

| # | 요청 | 한 것 |
|---|---|---|
| 1·7 | 옛 쿠키에 이메일·토큰이 남아 있다 / 이행기를 두지 말자 | **이행기 없이 끊었다.** 서버는 `snap_user` 안의 토큰을 더 읽지 않고(`readSessionTokenFromRequest`), 클라이언트 `saveSession` 은 토큰·전화번호·이메일을 떼고 저장(`stripSensitive`), 옛 형식을 읽으면 즉시 다시 쓴다. `SessionUser.phone` 필드와 화면의 `?phone=` 쿼리는 여섯 저장소에서 걷었다. **옛 세션은 한 번 다시 로그인**한다 |
| 2 | 비밀번호 찾기 메일이 안 온다 | 원인은 SMTP 가 아니라 **조회 조건** — `findOne({ phone, email })` 이라 전화번호 없는(이메일 가입) 계정은 영영 일치하지 않았다. 이제 **이메일만** 받는다. 재설정 링크는 PIN 이 아니라 **비밀번호**를 정한다(`reset-pin-email` 라우트·페이지, 경로 이름은 그대로) |
| 2 | 이메일로만 로그인 | `/api/auth/login` 이 전화번호 식별자를 400(`migrate: true`)으로 막고, 화면은 이메일·비밀번호 라벨 + "전화번호·PIN으로 쓰셨나요? → 전환" 링크. 전화번호 찾기 링크 제거 |
| 2 | 전화번호+PIN 회원의 전환 | **`/migrate`** + `/api/auth/migrate-pin` — 전화번호·PIN 으로 본인 확인 → 비밀번호 저장 · `pendingEmail` · `pinRetireOnVerify` · 인증 메일. `verify-email` 이 주소를 옮기며 **PIN 을 지우고** 세션 버전을 올린다. 시도 제한은 로그인과 같다. 같은 번호로 계정이 둘이면 409 + 관리자 메일 안내 |
| 2 | 관리자 안내 | 비밀번호 찾기·전환 화면에 `myjane0602@gmail.com` (`lib/contact.ts` 한 곳) — "이메일을 등록해 드리고 회신" |
| 3 | 크론 확인 가이드 | [[C 법적 페이지]] "크론 폐기 — 확인하는 방법" |
| 4 | 초안 알림 | `LegalShell` 에서 제거 |
| 5 | B7 문의 스냅샷 | 세 앱 문의 POST 가 이름·전화번호를 더 쓰지 않는다. admin 목록은 `userId` 로 회원을 조회해 이름·**이메일**을 붙인다(키 이름 `phone` 은 포털 화면 호환으로 유지). `Inquiry.name` 선택화 |
| 6 | Atlas 권한 분리 | **하지 않는다** — 1인 운영. [[MongoDB Atlas]] 의 체크박스는 참고로 남긴다 |
| 8 | 자녀 계정 | 계획서 필요 → [[F 보호자·자녀 계정]] (결정표 대기) |
| 9 | 2hbk R2 옛 키 | `scripts/migrate-r2-keys.mjs` — 참조 1건 이관(실패 0) · 고아 0건. Azure 시절 파일은 애초에 R2 에 없었다 |

### 2차 배포 확인 (2026-09-09 운영 · 커밋 myjane 76bcce9 · SnapWord 35f02a6 · SnapNote 459d24c · fitlog 74b96cd · 2hbk 0a98212 · typelog 74c0138)

```
전화번호 로그인            400 + migrate:true
비밀번호 찾기 이메일만     200 (있든 없든 같은 답) · 전화번호만 보내면 400
/migrate                  200 · 틀린 PIN 401
방침 초안 알림            0건 · fitlog 옛 update-email 410 · 다섯 앱 /api/me 무인증 401
```
로그인 페이지 문구는 클라이언트 렌더라 curl 로는 안 보인다 — 브라우저에서 확인.

### 남은 것 (2차 이후)

- [ ] 운영에서 **이메일로 비밀번호 찾기 → 메일 수신 → 새 비밀번호로 로그인** 한 번
- [ ] 전화번호 계정 4건 중 하나로 `/migrate` 를 실제로 돌려 PIN 이 지워지는지
- [ ] 앱 로컬 개발용 `/api/auth/login` 은 여전히 전화번호+PIN 을 받는다(운영에서는 포털만 쓴다). 정리 대상
- [ ] `/find-phone` 페이지는 링크만 지웠고 경로는 남아 있다 — 410 으로 닫기
- [x] [[F 보호자·자녀 계정]] — 결정 · 구현 (2026-09-09)
