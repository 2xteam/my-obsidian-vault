...
6. [x] 카드를 담은 **부모 시트**에도 라운딩 포인트 — 사용자 요청
7. [ ] 배포  ← **여기**
...
## 카드를 담은 부모 시트까지 확장 (2026-09-07)

사용자 요청: "안쪽에 카드가 있는 부모 카드이면 **부모 카드가** 좌상·우하 라운딩이
다른 UI 를 하고 **자식들은 일반적인 라운딩**".

### ⚠️ 처음에 반대로 만들었다

"안쪽에 카드가 더 있는 타입에도 라운딩을 쓰도록" 을 **자식 카드를 깎으라는
말로** 읽고, 부모 시트는 그대로 두고 안쪽 카드마다 `.card--point` 를 붙였다.
사용자가 바로잡았다 — "반대로 되었어요".

방향이 왜 그래야 하는지: 깎임이 **"이 시트는 안에 카드를 담고 있다"** 는
신호다. 자식까지 깎으면 부모와 자식이 같은 신호를 내서 계층이 사라진다.
부모만 깎고 자식은 평범하게 둬야 담고 있다는 관계가 보인다.

`.card--point` 는 지웠다. 인라인 `borderRadius` 를 longhand 로 쪼갰던 것도
되돌렸다.
...
| 앱 | 카드를 담은 시트 |
|---|---|
| myjane | 카테고리 4개(HEALTH · TYPE · STUDY · HABIT) · `ABOUT MYJANE` |
...
의미가 바뀌었기 때문이다. 강조 장식이면 "한 화면에 2개" 가 맞지만 **구조
신호**는 해당되는 시트가 여섯 개면 여섯 개 다 붙는 게 맞다. 개수로 막으면
규칙을 지킨 페이지가 실패한다. G 는 이제 형광 밑줄(`mark`)만 센다.
검사기 주석에 되살리지 말라고 적어 뒀다.

### 구현 노트

`border-radius` 를 통째로 쓰지 않고 **좌상·우하 longhand 두 줄만** 덮어쓴다.
나머지 두 모서리는 요소가 원래 갖던 값을 그대로 둔다 — 반경이 다른 요소
(시트 24px, 카드 10~18px)에 같은 규칙을 쓸 수 있다.

⚠️ 인라인 `borderRadius` **shorthand** 는 우선순위가 가장 높아 네 모서리를 모두
세우고 그 longhand 두 줄을 조용히 덮어쓴다. 인라인 스타일을 쓰는 요소에
붙일 때 주의할 것.
