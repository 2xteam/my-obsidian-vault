...
6. [x] 안쪽 카드에도 라운딩 포인트 (`.card--point`) — 사용자 요청
7. [ ] 배포  ← **여기**
...
## 안쪽 카드까지 확장 (2026-09-07)

사용자 요청: "안쪽에 카드가 더 있는 타입에도 왼쪽위·오른쪽하단 라운딩을 쓰도록".
내가 세운 "나란히 놓인 카드에는 넣지 않는다" 를 사용자가 뒤집은 것이다.
렌더해 보니 사용자 판단이 맞았다 — 상쇄가 아니라 **한 벌로 읽힌다.**
시트는 24px 로 둥글고 그 안의 카드만 같은 방향으로 깎이니 계층이 또렷해진다.

**구현에서 배운 것.** `border-radius` 를 통째로 쓰면 카드의 **원래 둥글기까지**
시트 기준으로 덮어쓴다 (시트 24px, 카드 10~18px). 그래서 두 요소 모두
**두 모서리만 longhand 로 덮어쓰게** 바꿨다 —
`border-top-left-radius` · `border-bottom-right-radius` 만 세우고 나머지 둘은
요소가 원래 갖던 값을 그대로 둔다. 반경이 다른 것들에 같은 규칙을 쓸 수 있게 됐다.

⚠️ 인라인 `borderRadius` **shorthand** 는 우선순위가 가장 높아 네 모서리를 모두
세우고 그 longhand 두 줄을 조용히 덮어쓴다. 다섯 앱의 `featureStyle`·`modeStyle`
에서 shorthand 를 빼고 남는 두 모서리만 longhand 로 적었다.

**깊이는 네 가지를 나란히 렌더해 골랐다** — 시트 6px · 카드 **3px**.
카드 반경의 1/4(2.5px)은 눈에 거의 안 보이고, 0px(직각)은 시트의 6px 과 너무
멀어 카드만 다른 언어를 쓰는 것처럼 보였다.

| 앱 | 붙인 카드 |
|---|---|
| myjane | 서비스 카드 5개 · `ABOUT MYJANE` 안내 카드 4개 |
| SnapWord · SnapNote | `WHAT YOU GET` 기능 카드 3개 |
| fitlog | `WHAT YOU GET` 기능 카드 6개 |
| 2hbk | `THREE MODES` 모드 카드 |
| typelog | 안쪽 카드가 없다 |
