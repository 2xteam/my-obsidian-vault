...
7. [x] 진행 띠를 결쩜사 방식(두 테마색 그라디언트)으로 · 모든 페이지에 적용
8. [ ] 배포  ← **여기**
...
## 진행 띠 개선 + 모든 페이지 적용 (2026-09-07)

### 결쩜사 실측

`kyulzzumsa.co.kr/process` 의 띠를 CDP 로 재 봤다.

```
position: fixed;  top: 60px;  height: 3px;  z-index: 299;
style="width: 45.0262%"                       ← 인라인 % 로 폭을 준다
background: linear-gradient(90deg, #7c3aed, #c9a84c)
```

핵심은 **그라디언트가 띠의 현재 폭에 맞춰 그려진다**는 것이다. 그래서 띠가
짧아도 양쪽 테마색이 늘 보이고 가운데만 섞인다. 고정 폭 그라디언트를 마스크로
드러내면 초반에 왼쪽 색만 보여 전혀 다른 느낌이 된다.

우리는 `scaleX` 를 쓰는데 **결과가 같다** — `transform` 이 배경까지 함께 누르기
때문이다. 게다가 레이아웃을 다시 계산하지 않아 스크롤 중에 더 싸다.
그래서 칠만 그라디언트로 바꿨다.

정지 렌더로 15·35·60·85·100% 를 나란히 놓고 픽셀을 재 확인했다 — 어느 진행률에서도
왼쪽 `#116271`(6.98:1) · 오른쪽 `#a8842f`(3.50:1) 로 양끝이 살아 있다.

### ⚠️ 오른쪽 끝에 --gold 를 쓸 수 없었다

`--gold`(`#c9a84c`)는 흰 헤더 위 **2.29:1** 이다. 비텍스트 기준 3.0 미달이고,
하필 **진행 위치를 알려주는 그 끝**이 가장 안 보이게 된다. 결쩜사는 같은 금색을
그대로 쓴다 — 레퍼런스가 통과하지 않는 자리다.

같은 금색 계열에서 내려 `#a8842f`(3.50:1)를 골랐다. 후보를 계산해 봤다:
`#b8923a` 2.91(미달) · `#ad8932` 3.28 · `#a8842f` 3.50 · `#9c7a2b` 4.01.
다크는 헤더가 어두우니 밝은 쪽이 맞아 `#e8c96a`(11.44:1)를 쓴다.

팔레트에 `progressFrom` / `progressTo` 역할을 추가하고 검사 3건을 걸었다.
**대비 관문이 그라디언트 칠을 검사하도록** 고쳤다 — 전에는 배경만 배열을
받았는데, 칠 자체가 그라디언트인 자리가 생겨서 앞쪽도 배열을 받게 했다
(모든 조합 중 최악을 본다).

### 모든 페이지에 넣었다

루트에만 넣었던 것을 지적받았다. 껍데기 컴포넌트에 넣어 전 페이지로 퍼뜨렸다.

| | 넣은 자리 |
|---|---|
| myjane | `AuthShell.tsx`(로그인·가입·이메일) · `LegalShell.tsx`(법적) · `app/page.tsx` |
| 다섯 앱 | `TopNav.tsx`(앱 화면 전체) · `app/page.tsx`(소개) |

`.auth-topbar` 는 sticky 가 아니었어서 `elements.css` 에서 sticky 로 만들었다.

⚠️ 앱 화면에서는 `.topnav` 이 아니라 **`.topnav-bar` 안**에 넣어야 한다. `.topnav` 은
햄버거를 열면 메뉴만큼 키가 자라서 띠가 메뉴 밑으로 내려간다. 처음에 `.topnav`
아래 변에 붙였다가 옮겼다. `.topnav-bar` 에 `position: relative` 를 준다.
닫힘·열림 두 상태를 정지 렌더로 확인했다.

**예외 둘** — `/admin`(별도 화면, `admin.css`) · typelog `app/r/[token]`
(상단 메뉴를 일부러 두지 않은 공유 링크 화면).

### ⚠️ Browser 패널이 CSS 를 아예 적용하지 않은 적이 있다

앱 화면을 재려는데 `.topnav` 의 `position` 이 `static`, `body` 배경이 투명으로
나왔다. CSS 를 잘못 쓴 줄 알았는데 —

- 서버가 준 `layout.css` 는 200 이고 규칙도 다 들어 있었다
- `document.styleSheets` 는 3개인데 `cssRules` 접근이 전부 blocked 였다
- **헤드리스 크롬으로 같은 URL 을 열면 정상**이었다

패널 쪽 상태 문제였다. 스타일이 통째로 안 먹는 것처럼 보이면 코드를 뒤지기 전에
**헤드리스로 한 번 열어 볼 것.** → [[개발 서버와 검증 환경]]
