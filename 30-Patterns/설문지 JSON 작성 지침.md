...
| 필드 | 동작 |
|---|---|
| `startAt` | 공개했어도 이 시각 전에는 **잠긴 카드**로 보인다. 없으면 공개 즉시 열린다 |
| `endAt` | 지나면 새 응시만 막는다. 지난 기록은 계속 볼 수 있다. 없으면 무기한 |
| `openingNoticeText` | 잠긴 동안 띄우는 문구. **없으면 기본 문구**가 나온다 |
...
```json
{
  ...
  "rarity": "common",
  "content": {
    "summary": "너는 친구가 무슨 생각을 하는지 잘 알아채는 아이야. ...",
...
| `content.summary` | ✅ | 2~3문장, 120자 이내. **아이에게 말하듯** |
...
- "검사·진단·성격유형" 대신 "놀이·성향". 단정하지 않는다
- 답을 문장에 넣을 수 있다 — `{{answer.<linkId>}}`.
  예: `"{{answer.animal_name}}는 다정한 돌고래야!"`
- 존댓말·반말을 한 설문지 안에서 섞지 않는다 (기본은 반말 — 아이에게 말하는 톤)
...
```json
{
  ...
  "resultTypes": [
    { "code": "dolphin", "name": "다정한 돌고래", "emoji": "🐬", "color": "#7c3aed",
      "content": {
        "summary": "너는 친구 마음을 먼저 알아채는 아이야. 같이 웃는 걸 제일 좋아해.",
        "strengths": ["친구를 잘 도와줘요", "함께 하는 놀이를 좋아해요"],
...
| 회차 비교 그래프가 이상하다 | 화면이 `raw`를 그리고 있다 | `pomp`로 그린다 → [[TypeLog]] |

## 관련

- [[TypeLog]] — 데이터 모델과 결정 사항
- [[OpenAI Vision 추출 패턴]] — 사람이 검토하지 않는 입력을 기계가 검사하는 같은 발상
- [[SVG 차트 패턴]] — POMP 추이·레이더 그리기
